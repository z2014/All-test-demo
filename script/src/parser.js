export default class Parser {
    constructor (props) {
        ...
        this.text = props.html.slice(3, props.html.length - 4),
        this._html = ''
    }
    render (txt) {
        ...
        switch (true) {
            ...
            case /\${\w+}/.test(txt):
                ...
                if (this.dataList[key] === undefined) {
                    num = 'NaN'
                    txt = txt.replace(/\${\w+}/, num)
                } else if (this.dataList[key] === "" || this.dataList[key] === null) {
                    txt = txt.replace(/\${\w+}/, "")
                } else if (this.dataList[key] === 0) {
                    txt = txt.replace(/\${\w+}/, 0)
                } else if (isNaN(this.dataList[key])) {
                    num = this.dataList[key]
                    if (isopt) {
                        txt = txt.replace(/\${\w+}/, `'${num}'`)
                    } else {
                        txt = txt.replace(/\${\w+}/, num)
